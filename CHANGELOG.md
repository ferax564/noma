...
- **Chat (Noma Cloud):** Slack-style channels inside each space, organised by Work project or by topic, with public and private channels, threads, reactions, edits, read markers, and unread/mention counts.
  ...
  - **UI:** a Slack-style chat drawer, opened from the **Chat** launcher in the inspector (which lists channels with unread and mention badges).
    - **Layout:** a channel sidebar grouped into Projects and Topics, with message search and a new-channel form. A thread pane opens beside the timeline.
...
- **Cloud search filters:** `/api/search` and `/api/knowledge/search` accept `label=`, `author=`, `space=`, `updatedAfter=`/`updatedBefore=`, and `type=` (page or block/directive name), plus the same filters and `"exact phrases"` inside `q` (`label:how-to author:@ada space:ENG`). Filter-only queries list matching pages. The search panel gains type/date/label/author dropdowns and removable filter chips.
- **Cloud @mentions:** a mention picker in comment boxes and the source editor (`GET /api/users?q=` searches only people who share a space with you; `?ids=` resolves names), `@{userId}` shown as `@Name` in comments and the preview, and page-source mentions that notify newly mentioned users with access on save.
- **Cloud comments:** authors can edit (`PATCH …/comments/<id>`, shown as *edited*) and soft-delete (`DELETE …/comments/<id>`; page owners and workspace admins can delete others') comments without breaking threads; emoji reactions (`POST/DELETE …/reactions`); and text-range anchors (`anchor: {blockId, quote, prefix, suffix}`) that the preview highlights and that turn `outdated` instead of disappearing when the quote is edited away.
- **Cloud space settings:** unique uppercase space keys (derived from the title when omitted), description, icon, and a home page that the app opens first and `/s/<id>` renders first; owners can archive/unarchive spaces (`POST /api/sites/<id>/archive|unarchive`), which makes them read-only (`409 space_archived`), hides them from `GET /api/sites` unless `?archived=include`, and drops their pages from search unless the query says `archived:include` or names the space. The Cloud UI adds a Space settings panel and an Archived toggle in the Spaces rail.
- **Cloud page analytics:** page views recorded by a view beacon and `/d/<id>` (deduplicated per viewer per page for 30 minutes; share-link views anonymous), `GET /api/documents/<id>/analytics` (views per day, unique viewers; the viewer list only for editors and owners), and `GET /api/sites/<id>/popular`. The page header shows *N views* and the rail lists popular pages in the space.
- **Cloud page tree reordering:** drag and drop pages in the rail (before, after, or into a page), with keyboard (Alt+Arrow) and context-menu Move up/down, Indent, and Outdent. `PUT …/parent` with a `position` past the last sibling now appends instead of leaving the page in place.
- **Cloud inline tasks:** `- [ ] text @{user} due:YYYY-MM-DD` list items are tracked tasks. Human saves give them stable `{#task-…}` IDs in the source; every save re-indexes them into `page_tasks` and notifies newly assigned users (`task_assigned`). `GET /api/tasks` lists My tasks across spaces, and `POST /api/documents/<id>/tasks/<task-id>` completes a task through a hash-checked block patch. The rail shows My tasks and preview checkboxes are clickable.
- **Cloud outbound webhooks:** space owners register webhooks (`POST/GET/DELETE /api/sites/<id>/webhooks`) for `page.created`, `page.updated`, `page.deleted`, `comment.created`, `label.changed`, and `task.completed`. Deliveries are HMAC-SHA256 signed (`x-noma-signature`), queued in SQLite, retried with exponential backoff by an in-process timer or `apps/worker/cloud-queue.ts`, and listed in a delivery log; `format: "slack"` posts `{text}` to Slack incoming webhooks. The Space settings panel manages them.
- **Cloud email and digests:** a private profile email (`PUT /api/users/me`), per-type notification channels (in app, in app + email, or off) and daily/weekly digests of unread notifications (`GET/PUT /api/users/me/preferences`). Mail is sent from a persisted outbox with retries through a built-in SMTP client (implicit TLS or STARTTLS, AUTH PLAIN/LOGIN; `NOMA_CLOUD_SMTP_URL`) or a `log` transport for development. The Notifications panel gains Notification settings.
- **Noma Cloud attachments and page restrictions:** pages carry files in content-addressed blob storage, with dedupe, magic-byte type sniffing, executable rejection, a 25 MB per-file limit, and a 1 GB per-space quota. Pages reference files as `att:<id>` or `att:<filename>`, which Cloud renders through short-lived signed URLs checked again on every request. Attachments are indexed in search, included in backups, and garbage-collected when a page is purged. The editor gains an Attachments panel and drag-and-drop or paste upload. Confluence-style view and edit restrictions (`GET/PUT /api/documents/<id>/restrictions`) narrow space grants per page. View restrictions are inherited down the page tree and enforced in every read, search, knowledge, agent, share-link, notification, attachment, and published-site path. The UI adds a Restrictions dialog and lock badges in the page header and tree.
- **Wiki macros, templates, Confluence import, and export:** new `::include` (block, excerpt, or whole-page transclusion with a permission check, cycle guard, and a nesting limit of three), `::excerpt`, `::children`, `::issue`, `::issues`, `::page-properties`, and `::page-properties-report` directives. They resolve through optional render-time resolvers (`HtmlRenderOptions` / `RenderLlmOptions`), and the LLM renderer adds `<!-- included from doc:block@hash -->` provenance. Noma Cloud adds `POST /api/macros/resolve` for the preview, plus page summaries in the tree and search. Workspace and space templates are stored in `page_templates`, with `/api/templates` CRUD and `{{variable}}` blueprints whose values are escaped. `POST /api/import/confluence` imports a Confluence space in the background from live Cloud/Data Center credentials, an XML space export, or a JSON bundle, keeping hierarchy, labels, and provenance, and re-imports idempotently (`GET /api/import/jobs/:id`). Exports: `GET /api/documents/:id/export?to=pdf|docx|markdown|html|noma|llm|json` and `GET /api/sites/:id/export?to=site-zip|noma-zip`. The UI adds an Import-from-Confluence dialog, a Save-as-template button, a variables form, and an Export menu.
- **Noma Cloud visual editor and live co-editing:** a Visual mode (the default for new users; each user's choice is remembered) with a ProseMirror block editor that maps 1:1 to Noma blocks. It has a slash menu, a floating toolbar, Markdown shortcuts, sanitized HTML/Markdown paste through the ingest pipeline, attribute editors for directives, and raw-source cards for blocks it cannot represent. `src/editor-model.ts` converts `.noma` to and from editor JSON and copies untouched blocks byte for byte. Every example and doc round-trips exactly, stable IDs are kept, and new blocks get deterministic IDs. A Yjs relay at `/api/collab/documents/<id>` stores each update before acknowledging it, shows presence and cursors, re-checks permissions, writes coalesced checkpoint revisions, and merges agent patches and API writes into the live document block by block.
...
- **Breaking: lean root entry and subpath exports.** `import … from "@ferax564/noma-cli"` now exposes only the core format API (AST, parser, renderers, validator, patch, proof, ids, diff, fmt, DOCX helpers, stable identity) and no longer loads `better-sqlite3`, `ws`, or `yjs`. Cloud APIs moved to `@ferax564/noma-cli/cloud` (new `src/cloud.ts` barrel: `createNomaCloudServer`, `NomaCloudDatabase`/`openNomaCloudDatabase`, `CloudKnowledgePlatform`, `cloudPageTemplates`, and their types) and enterprise APIs to `@ferax564/noma-cli/enterprise`. Migration: change `import { CloudKnowledgePlatform } from "@ferax564/noma-cli"` to `from "@ferax564/noma-cli/cloud"`, and `import { EnterpriseWorkspace, … } from "@ferax564/noma-cli"` to `from "@ferax564/noma-cli/enterprise"`. The extraction-boundary packages (`noma-platform`, `-work`, `-knowledge`, `-connectors`, `-contracts`, `-document-ui`, `-paperdom-*`) re-export from the enterprise subpath; `noma-document-core` stays on the lean root. A test and `npm run smoke:package` guard that the root import does not pull in native deps.
- **Puppeteer is an optional peer dependency.** `--to pdf` still uses it, but a plain `npm i @ferax564/noma-cli` no longer implies it. Without it, `--to pdf` fails with `install puppeteer to render PDF: npm i puppeteer`.
- **PR CI enforces Iron Rule 5 and the Python suites:** `ci.yml` now renders all examples and docs (Ubuntu / Node 22 leg), runs `typecheck:web` when present, and runs the Python agent SDK (`pytest`) and `noma-py-seed` conformance tests (new `npm run test:python-seed`, also part of `test:full`).
- **Enterprise Yjs relay hardened:** `/yjs` authenticates from `Authorization: Bearer` or a `noma.bearer.<token>` WebSocket subprotocol (the relay selects `noma.v1`). The `?token=` query parameter only works behind `allowQueryToken`, which is off by default, and the browser client moves the token into the URL fragment. The relay evicts idle rooms, compacts persisted updates into one snapshot in a transaction (`compactYjsUpdates`), allocates sequence numbers inside `BEGIN IMMEDIATE` so it is safe across processes, rejects malformed updates before they reach disk, relays presence with server-asserted identity, and admits viewers read-only. `attachEnterpriseYjs` now returns a relay handle (`wss`, `roomIds()`, `presence()`, `close()`). `persistYjsUpdate`'s fifth argument is now an options object with a test-only `hooks.afterPersist`, replacing `simulateLostAck`.
- **`enterprise-crdt` renamed to `enterprise-merge` (block-level three-way merge):** `applyBlockOps`, `detectBlockConflicts`, and `mergeBlockEdits` edit source in place through `patchSource` instead of re-rendering the document. Identical concurrent edits no longer conflict. `applyCrdtOps`, `crdtOpsConflict`, and `CrdtOp` remain as deprecated aliases. `persistCollaborativeUpdate` takes a hooks argument instead of `simulateLostAck`.
- **Atlassian connector pagination:** `listConfluencePages` (`start`/`limit`/`_links.next`) and a paginated `searchJira` (Cloud `nextPageToken`, Data Center `startAt`/`total`), both with a `maxPages` guard. Confluence Cloud now uses `/wiki/rest/api` and Data Center uses `/rest/api`. Requests are pinned to the configured origin, and `trustedPrivateHosts` is an explicit opt-out for on-prem hosts.
- **AWS/EU CloudFormation is deployable:** RDS PostgreSQL now has an instance class, storage, engine version, a master username with an RDS-managed password, a subnet group, and a security group. An EU-region `Rules` assertion is added, the S3 bucket is TLS-only, and Bedrock is an optional `BedrockInferenceProfileId` parameter. `assertAwsEuStack` validates required properties for each resource type and catches dangling references.

### Security

- Outbound webhooks reject loopback, private, link-local, CGNAT, multicast, and unique-local targets, and re-check resolved addresses on every delivery (DNS rebinding). `NOMA_CLOUD_ALLOW_PRIVATE_WEBHOOKS=1` opts out for on-premises targets.
...
- **Noma Cloud auth hardening:** the web app now signs in with an HttpOnly `noma_session` cookie backed by a hashed server-side sessions table, and cookie-authenticated writes require the `X-Noma-CSRF` double-submit header (`GET /api/auth/session`, `GET /api/auth/sessions`, `DELETE /api/auth/sessions/<id>`, `POST /api/auth/logout`); raw tokens are no longer kept in `localStorage`, and old stored tokens are exchanged for a session and deleted on first load. Named personal access tokens (`POST|GET /api/tokens`, `DELETE /api/tokens/<id>`, prefix `noma_pat_`) are hashed, scoped (`read`/`write`/`admin`), expiring, and track last use; the legacy user token keeps full scope and can be rotated with `POST /api/users/me/rotate-token`. Production enterprise admin fails closed (`403 admin_not_configured`) without `NOMA_CLOUD_ADMIN_USER_IDS`; `?access=` gate checks use the auth rate limiter on every path; token previews are visible only to their owner; backup imports validate the whole bundle, write in one transaction, and return a uniform `409 backup_ids_unavailable`; knowledge requests cap documents and cache indexed blocks by content hash; platform listings paginate; offline drafts and analytics have per-user quotas; legacy JSON records move to `legacy-imported-<timestamp>/` after import.
...
- **fmt:** `noma fmt` recognises the same code fences as the parser (`~~~`, fences of four or more backticks, any info string), so tables inside them are left alone.
...
- The native Python seed (`packages/noma-py-seed`) implements the new parser and patch rules and passes all 25 fixtures it covers, including the new ones.
- The agent SDKs (`@ferax564/noma-agent-sdk`, `noma-agent-sdk` for Python) list `invalid_attribute_key`, `invalid_attribute_value`, and `unbalanced_fence_content` in `PatchErrorCode`, with parity tests against `src/patch.ts`.
...
- **Live Atlassian HTTP:** Confluence/Jira Cloud (Basic) and Data Center (Bearer PAT) adapters with SSRF checks and `429`/`Retry-After` retries, wired into `importLiveConfluencePage` / `importLiveJiraIssue`.
...
- **Connected knowledge and external agent gateway:** persisted GitHub, Slack, Google Drive, Jira, Linear, and filesystem connector records retain upstream permissions, modified time, URL, tombstone, and lineage. Six proposal-only maintenance recipes, typed cross-block collections, permission-safe analytics, deterministic `.noma` backup/import conflict plans, REST/webhook operations, and a JSON-RPC MCP `tools/list`/`tools/call` gateway cover P1 without bypassing proof, independent review, or hash-checked apply.
- **Offline, realtime, and enterprise Cloud depth:** the Cloud UI is an installable PWA with cached shell, full local draft snapshots, automatic safe recovery, and explicit three-way conflict merging. Human-only ordered realtime operations reuse stable IDs, proofs, hashes, and immutable revisions. Enterprise policy adds trusted-proxy OIDC/SAML login, SCIM, retention, legal holds, data residency, audit export, connector/model allowlists, zero-retention enforcement, and per-agent spend limits.
- **Agent-human platform verification:** a focused service suite covers exact retrieval, leakage, stale evidence, abstention, health, wiki, agents, connectors, recipes, analytics, backup, offline merge, realtime ordering, and enterprise controls; the Cloud API integration exercises the end-to-end trust loop and MCP transport; the browser regression now covers hybrid search, cited Ask, agent/health/inbox panels, service-worker installation, local draft caching, and visible merge conflicts.
- **Agent-human knowledge platform research and roadmap:** a new Noma-authored market scan compares Confluence/Rovo, Notion, Glean, Guru, and Slite; separates current capabilities from user pain; records Notion's most requested feature themes; and proposes a phased Noma Cloud direction around block-native hybrid RAG, versioned citations, LLM Wiki maintenance, knowledge verification, scoped agent identities, evaluation fixtures, connectors, semantic collections, offline continuity, and realtime/enterprise sequencing. PLAN.md §26 makes the trust loop explicit: permission-scoped retrieval → cited answer/abstention → proofed patch proposal → independent human approval → hash-checked apply.
...
- **v0.16 spec audit — freeze the full surface.** Decision: graduate the full patch-op catalog, the `baseHash` block precondition, and Agent Protocol Annexes A (capability descriptor) and B (MCP-over-stdio binding) from provisional/extension status to **normative**, closing v1.0 exit criterion §25.3 #4.
  ...
  - **Format spec.** `docs/spec.noma` adds a "Stability and the road to v1.0" summary pointing at the register; its stale `0.13.0` body version is corrected to `0.16.0`.
...
- **Computed tables:** `::computed_table{formula="..." domain="x:0..5"}` now renders static HTML tables, recalculates in standalone interactive HTML, exports as native captioned Word tables, participates in LLM default-series output, validates under the computed formula rules, and is accepted by the technical/research profiles.
- **Computed-table DOCX review return path:** Word review extraction and sync now recognize computed-table captions and formula/domain/unit metadata, preserving literal formula operators and mapping accepted Word edits back to `label=` / `title=` / body fields and metadata attrs source-preservingly.
- **Interactive scenario URL state:** standalone computed artifacts now encode current control values in a `#noma:` URL hash and restore them on load, so rendered projections can be shared without a server.
- **Interactive projection demo:** added `examples/interactive-projection.noma`, rendered as HTML/LLM/JSON in the normal example build, to exercise controls, computed metrics, computed plots, computed tables, and URL-hash scenario sharing.
- **Word review-loop demo:** added `examples/word-review-loop.noma`, rendered as HTML/LLM/JSON/DOCX in the normal example build, to demonstrate source-controlled review controls, comments, change requests, and extractable Word return data.
- **Python SDK publish runbook:** added maintainer steps for building, checking, publishing, and verifying the unpublished Python starter binding on PyPI.
...
- **HTML lateral layout controls:** `::grid` and `::columns` now accept `min=`, `min_width=`, `minWidth=`, `min-width=`, `minColumnWidth=`, `gap=`, `wide`, `full`, `width="wide|full"`, `span="wide|full"`, `compact`, and `dense` for denser side-by-side report and dashboard layouts.
