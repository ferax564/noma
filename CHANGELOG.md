...
- **Enterprise workspace product UI:** a Docs / Visuals / Work / Admin shell that copies Confluence Cloud chrome in detail — 56px top bar, app switcher, brand mark, mode tabs, gray search pill, blue Create, space sidebar with page tree, 760px page canvas, breadcrumbs, title/byline, sticky editor toolbar, comment threads, and a Details panel — served at `/` from the enterprise HTTP server. The hosted shell uses professional libraries that stay out of the published CLI: Lucide icons, Floating UI for search, slash menus, selection bubbles, and selects, pointer-captured board drag (no Atlassian packages, no HTML5 drag clones), Tiptap tables, task lists, highlight, text align, in-editor `@` mentions with a Floating UI picker, character/word counts, find-in-page, selection comments that quote the highlighted span, Noma info/warning/claim panels, live collab presence avatars and Yjs awareness carets, page covers and an On this page outline, an interactive Visuals canvas with pan/zoom/colored sticky notes, comment pins, resize handles, click-to-connect arrows, canvas undo, delete, front/back stacking, export fidelity, and present mode, a Ctrl/⌘K command palette, @mention pickers on comments, and a Jira-style issue panel with reporter, parent/epic, children, related-issue links, watchers, flags, priority, due dates, labels, estimates, activity, unassigned/overdue/flagged/watching filters, epic filter, epic swimlanes, board/list/reports/backlog views (throughput, cycle time, cumulative flow, and sprint burndown), type filters, and issue search.
- **Enterprise product surface (Confluence/Jira substitution slice):** HTTP and UI for the page tree (create/rename/move), comments and `@mentions`, attachments, version history restore, Confluence import with a visible macro loss report, issue create/detail with real workflow transitions, sprints, a JQL bar that reports unsupported fields, worklog and rank, a notification inbox, and Admin for spaces/projects/grants. PostgreSQL is a selectable running store; Noma Cloud is documented as the knowledge-platform preview, not a second substitution product.
- **Enterprise authoring loop:** labeled workspace and page creation, page/workspace permission grants, page media (`::figure` / `::video`), presentation boards with frames/arrows/images/videos, comments, and links to native work issues plus `https://github.com/...` URLs.
