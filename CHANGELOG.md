...
- **Enterprise workspace product UI:** a Docs / Visuals / Work / Admin shell that copies Confluence Cloud chrome in detail — 56px top bar, app switcher, brand mark, mode tabs, gray search pill, blue Create, space sidebar with page tree, 760px page canvas, breadcrumbs, title/byline, sticky editor toolbar, comment threads, and a Details panel — served at `/` from the enterprise HTTP server. The hosted shell uses professional libraries that stay out of the published CLI: Lucide icons, Floating UI for search, slash menus, and selects, pointer-captured board drag (no Atlassian packages, no HTML5 drag clones), Tiptap tables, task lists, highlight, and Noma info/warning/claim panels, live collab presence avatars, an interactive Visuals canvas with pan/zoom/sticky notes and pointer-drag frames, a Ctrl/⌘K command palette, @mention pickers on comments, and a Jira-style issue panel with priority, activity, type filters, and issue search.
