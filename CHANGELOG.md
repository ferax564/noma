...
- **Enterprise workspace product UI:** a Docs / Visuals / Work / Admin shell that copies Confluence Cloud chrome in detail — 56px top bar, app switcher, brand mark, mode tabs, gray search pill, blue Create, space sidebar with page tree, 760px page canvas, breadcrumbs, title/byline, sticky editor toolbar, comment threads, and a Details panel — served at `/` from the enterprise HTTP server. The hosted shell uses professional libraries that stay out of the published CLI: Lucide icons, Floating UI for search, slash menus, selection bubbles, and selects, pointer-captured board drag (no Atlassian packages, no HTML5 drag clones), Tiptap tables, task lists, highlight, in-editor `@` mentions with a Floating UI picker, character/word counts, Noma info/warning/claim panels, live collab presence avatars and Yjs awareness carets, page covers and an On this page outline, an interactive Visuals canvas with pan/zoom/colored sticky notes, resize handles, click-to-connect arrows, and canvas undo, a Ctrl/⌘K command palette, @mention pickers on comments, and a Jira-style issue panel with reporter, parent/epic, priority, due dates, labels, estimates, activity, unassigned/overdue filters, epic swimlanes, board/list views, type filters, and issue search.
