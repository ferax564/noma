...
import { resolveSessionUser, resolveTokenUser } from "./cloud/security.js";
...
interface CollabClient {
  ...
  ws: WebSocket;
  principal: Principal;
  /** Re-resolves the socket's credentials so revoked sessions and tokens drop the connection. */
...
export class CloudCollabHub {
  ...
  private handleConnection(ws: WebSocket, req: IncomingMessage, url: URL, documentId: string): void {
    ...
    ws.on("message", (raw: RawData, isBinary: boolean) => {
      ...
      if (!client) {
        ...
        client = this.join(ws, req, documentId, clientId, userToken, shareToken);
        return;
      }
  ...
  private join(
    ...
    const authenticate = (): Principal => {
      ...
      if (userToken) {
        ...
        const resolved = resolveTokenUser(this.config, userToken);
        if (resolved) {
          principal.user = resolved.user;
          principal.auth = resolved.auth;
        }
      } else if (headerValue(req, "origin")) {
        const resolved = resolveSessionUser(this.config, req);
        if (resolved) {
          principal.user = resolved.user;
          principal.auth = resolved.auth;
        }
    ...
    try {
      ...
      access = scopedAccess(principal, requireRecordAccess(this.config, record, principal, "viewer"));
    } catch (error) {
  ...
  private recheck(client: CollabClient, room: CollabRoom, record: CloudDocumentRecord | undefined): void {
    ...
    if (record && !this.config.store.isTrashed("document", record.id)) {
      const principal = client.authenticate();
...
/** A token or session without the `write` scope may watch a live page but never edit it. */
function scopedAccess(principal: Principal, access: AccessContext): AccessContext {
  if (principal.auth && !principal.auth.scopes.includes("write") && access.role !== "viewer") return { ...access, role: "viewer" };
  return access;
}
