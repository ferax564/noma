...
(() => {
  ...
  function assertFragmentDepth(content, expected, op) {
...
::`;
  ...
  // src/slides.ts
  var SLIDE_LAYOUTS = ["title", "section", "content", "two-column", "statement", "quote", "media", "blank"];
  var DECK_ASPECTS = {
    "16:9": { width: 1280, height: 720 },
    "4:3": { width: 1024, height: 768 },
    "1:1": { width: 1080, height: 1080 }
  };
  function isSlideLayout(value) {
    return typeof value === "string" && SLIDE_LAYOUTS.includes(value);
  }
  function slideLayout(node) {
    return isSlideLayout(node.attrs.layout) ? node.attrs.layout : "content";
  }
  function deckAspect(node) {
    const aspect = node?.attrs.aspect;
    return typeof aspect === "string" && DECK_ASPECTS[aspect] ? aspect : "16:9";
  }
  function isDirective2(node, name) {
    return node.type === "directive" && node.name === name;
  }
  function deckSlides(deck) {
    return deck.children.filter((child) => isDirective2(child, "slide"));
  }
  function slideParts(slide) {
    const notes = slide.children.filter((child) => isDirective2(child, "notes"));
    const rest = slide.children.filter((child) => !isDirective2(child, "notes"));
    const explicit = typeof slide.attrs.title === "string" && slide.attrs.title.trim() ? slide.attrs.title : void 0;
  ...
  function findDecks(doc) {
    const out = [];
    const visit = (nodes) => {
      for (const node of nodes) {
        if (isDirective2(node, "deck")) out.push(node);
  ...
  // src/components.ts
  var CORE_DIRECTIVES = /* @__PURE__ */ new Set([
    "abstract",
    "accordion",
    "adr",
    "agent_task",
  ...
  var COMPONENT_NAME_RE = /^[a-z][a-z0-9_]{1,40}$/;
  var PROP_NAME_RE = /^[a-zA-Z_][\w-]{0,40}$/;
  var PLACEHOLDER_RE = /\{\{\s*([a-zA-Z_][\w-]*)\s*\}\}/g;
  var SLOT_PLACEHOLDER_RE = /^\{\{\s*slot(?::([a-zA-Z_][\w-]*))?\s*\}\}$/;
  var USE_RESERVED_ATTRS = /* @__PURE__ */ new Set(["id", "class", "noverify"]);
  var MAX_COMPONENT_DEPTH = 8;
  var MAX_COMPONENT_EXPANSIONS = 500;
  function readComponentDefinition(node, origin) {
  ...
  function componentKitFrom(sources) {
    const kit = /* @__PURE__ */ new Map();
    const issues = [];
  ...
  function expandList(nodes, kit, options, stack, budget) {
    let changed = false;
    const out = [];
    for (const node of nodes) {
  ...
  function expandNode(node, kit, options, stack, budget) {
  ...
  function adoptUseIdentity(expanded, use) {
    const root = expanded.length === 1 ? expanded[0] : void 0;
    ...
    if (next.type === "directive") {
      ...
      const classes = [attrs.class, use.attrs.class].filter((value) => typeof value === "string" && value.length > 0);
      if (classes.length > 0) attrs.class = classes.join(" ");
      next.attrs = attrs;
    }
    return [next];
  }
  function componentError(node, message) {
    return { type: "directive", name: "callout", attrs: { tone: "warning" }, body: `::${node.name} \u2014 ${message}`, children: [], ...node.pos ? { pos: node.pos } : {} };
  }
  function useBindings(definition, use) {
    const props = /* @__PURE__ */ new Map();
    for (const prop of definition.props) {
      const value = use.attrs[prop.name];
      props.set(prop.name, value === void 0 || value === true ? value === true ? "true" : "" : String(value));
    }
    const slots = /* @__PURE__ */ new Map([["", []]]);
    const bodyNodes = use.children.length === 0 && use.body ? [{ type: "paragraph", content: use.body }] : use.children;
    for (const child of bodyNodes) {
      if (child.type === "directive" && child.name === "slot") {
        ...
        const content = child.children.length === 0 && child.body ? [{ type: "paragraph", content: child.body }] : child.children;
  ...
  function instantiate(definition, use) {
    const { props, slots } = useBindings(definition, use);
    const idPrefix = use.id;
    const text = (value) => value.replace(PLACEHOLDER_RE, (whole, name) => props.has(name) ? props.get(name) : whole);
    const scopedId = (id) => id && idPrefix ? `${idPrefix}--${text(id)}` : void 0;
    const cloneList = (nodes) => nodes.flatMap(cloneNode);
    const cloneNode = (node) => {
      ...
      switch (node.type) {
        case "section": {
          const section = { type: "section", level: node.level, title: text(node.title), children: cloneList(node.children) };
        ...
        case "list":
          return [withId({ type: "list", ordered: node.ordered, items: node.items.map((item) => ({ type: "list_item", content: text(item.content) })) }, scopedId(node.id))];
        case "quote":
        ...
        case "directive": {
          ...
          const id = scopedId(node.id ?? (typeof node.attrs.id === "string" ? node.attrs.id : void 0));
          if (id) attrs.id = id;
          const clone = { type: "directive", name: node.name, attrs, children: cloneList(node.children) };
          if (node.body !== void 0) clone.body = text(node.body);
  ...
  function checkComponentUses(doc, kit) {
    const issues = [];
    const visit = (nodes) => {
      for (const node of nodes) {
        ...
        if (node.type === "directive" && !CORE_DIRECTIVES.has(node.name) && kit.has(node.name)) {
          ...
          for (const prop of definition.props) {
            if (!prop.optional && (node.attrs[prop.name] === void 0 || node.attrs[prop.name] === "")) {
  ...
  function componentSignature(definition) {
  ...
  var STYLE_TOKENS = new Set(Object.values(STYLE_TOKEN_GROUPS).flat());
  var STYLE_TOKEN_CLASS_PREFIX = "n-";
  var MAX_STYLE_TOKEN_ALIASES = 64;
  var ALIAS_NAME_RE = /^[a-z][a-z0-9-]{1,39}$/;
  function normalizeStyleTokenAliases(input) {
    const aliases = {};
    const errors = [];
    if (input === void 0 || input === null) return { aliases, errors };
    ...
    for (const [name, value] of entries.slice(0, MAX_STYLE_TOKEN_ALIASES)) {
      ...
      const words = Array.isArray(value) ? value.map(String) : typeof value === "string" ? value.split(/[\s,]+/).filter(Boolean) : void 0;
      ...
      aliases[name] = [...new Set(words)];
    }
    return { aliases, errors };
  }
  function resolveStyleTokenAliases(frontmatter, host) {
    return { ...normalizeStyleTokenAliases(frontmatter).aliases, ...host ?? {} };
  }
  function parseStyleTokens(value, aliases = {}) {
    const tokens = [];
    const unknown = [];
    ...
    for (const word of value.split(/[\s,]+/)) {
      if (!word) continue;
      if (STYLE_TOKENS.has(word)) add(word);
  ...
  function styleTokenClassNames(attrs, aliases = {}) {
    return parseStyleTokens(attrs.class, aliases).tokens.map((token) => `${STYLE_TOKEN_CLASS_PREFIX}${token}`);
  }

  // src/canvas-svg.ts
  var MAX_CANVAS_SOURCE_BYTES = 2 * 1024 * 1024;
  function readCanvasDocument(json2) {
    if (json2.length > MAX_CANVAS_SOURCE_BYTES) return { ok: false, error: `canvas JSON is larger than ${MAX_CANVAS_SOURCE_BYTES} bytes` };
    let value;
    try {
  ...
  function canvasPageSize(page) {
    const size = isRecord(page.size) ? page.size : {};
    return { width: bounded(size.width, 1280, 16, 2e4), height: bounded(size.height, 720, 16, 2e4) };
  }
  function canvasPageSvg(page, options = {}) {
    const { width, height } = canvasPageSize(page);
    const prefix = safeIdPart(options.idPrefix ?? "canvas");
    const label = options.label ?? str2(page.name) ?? "Canvas page";
    const elements = (Array.isArray(page.elements) ? page.elements : []).filter((el) => isRecord(el) && !el.hidden).map((el, index) => ({ el, index })).sort((a, b) => num(a.el.z, 0) - num(b.el.z, 0) || a.index - b.index).map(({ el }) => el);
    const byId = new Map(elements.map((el) => [str2(el.id) ?? "", el]));
    const body = elements.map((el, index) => renderElement(el, `${prefix}-${index}`, byId, options)).join("");
    const background = color(isRecord(page.background) ? page.background.color : void 0, "#ffffff");
  ...
  function canvasPages(document2, pageId) {
    const pages = document2.pages.filter((page) => isRecord(page));
    if (pageId) return pages.filter((page) => page.id === pageId);
    return pages.filter((page) => !page.hidden);
  }
  function canvasOutline(document2, pageId) {
    return canvasPages(document2, pageId).map((page, index) => {
      const elements = (Array.isArray(page.elements) ? page.elements : []).filter((el) => isRecord(el) && !el.hidden).slice().sort((a, b) => rowBand(a, page) - rowBand(b, page) || frameOf(a).x - frameOf(b).x);
      const lines = [];
      for (const el of elements) {
        ...
        if (isRecord(el.chart)) {
          ...
          lines.push(`${str2(chart.title) ?? "Chart"}: ${labels.map((label, i) => `${String(label)} ${String(values[i] ?? "")}`).join(", ")}`);
          continue;
        }
        const text = elementText(el);
        if (text) lines.push(...text.split("\n").map((line) => line.trim()).filter(Boolean));
        else if (el.type === "image" && isRecord(el.content) && str2(el.content.alt)) lines.push(`[image: ${str2(el.content.alt)}]`);
      }
      return { page: str2(page.name) ?? `Page ${index + 1}`, lines };
    });
  }
  function rowBand(el, page) {
    const f = frameOf(el);
    const band = canvasPageSize(page).height / 12;
    return Math.floor((f.y + f.h / 2) / band);
  }
  function renderElement(el, uid, byId, options) {
    ...
    const attrs = `data-element="${escapeAttr(str2(el.id) ?? "")}"${opacity < 1 ? ` opacity="${fmt(opacity)}"` : ""}${transform}`;
    switch (el.type) {
      case "line":
      case "connector":
      ...
      default:
        return `<g ${attrs}>${placeholder(frame, str2(el.name) ?? String(el.type ?? "element"))}</g>`;
    }
  }
  function boxMarkup(frame, style) {
  ...
  function textBox(el, frame, style, defaultAlign = "top") {
  ...
  function lineMarkup(el, frame, style, byId) {
  ...
  function endpoint(value, byId) {
    ...
    const target = typeof value.elementId === "string" ? byId.get(value.elementId) : void 0;
    if (target) {
  ...
  function tableMarkup(el, frame, style) {
    const table = isRecord(el.table) ? el.table : void 0;
  ...
  var CHART_COLORS = ["#2f6fa7", "#b9522a", "#2f7048", "#8a5cb8", "#9a681f", "#0f666b"];
  function chartMarkup(el, frame, style, uid) {
    const chart = isRecord(el.chart) ? el.chart : void 0;
    ...
    const yOf = (value) => bottom - (value - min) / span * plotH;
    ...
    const seriesColor = (index) => colors[index] || CHART_COLORS[index % CHART_COLORS.length];
    if (chart.kind === "line") {
      series.forEach((s, si) => {
  ...
  function chartSeries(chart) {
    const toValues = (raw) => Array.isArray(raw) ? raw.map((v) => typeof v === "number" && Number.isFinite(v) ? v : 0) : [];
    if (Array.isArray(chart.series) && chart.series.length > 0) {
      return chart.series.filter(isRecord).map((s, i) => ({ name: str2(s.name) ?? `Series ${i + 1}`, values: toValues(s.values) }));
    }
    const values = toValues(chart.values);
    return values.length ? [{ name: str2(chart.title) ?? "Series 1", values }] : [];
  }
  function imageMarkup(el, frame, options) {
    const content = isRecord(el.content) ? el.content : {};
    const src = str2(content.src);
    const alt = str2(content.alt) ?? str2(el.name) ?? "image";
    const href = src ? SAFE_IMAGE_DATA.test(src) ? src : options.resolveImage?.(src) : void 0;
  ...
  function paragraphsOf(el) {
    const content = isRecord(el.content) ? el.content : void 0;
    if (!content) return [];
    ...
    const text = str2(content.text) ?? str2(content.label);
    if (!text) return [];
    return text.split("\n").map((line) => ({ text: line, kind: "plain", level: 0 }));
  }
  function elementText(el) {
    return paragraphsOf(el).map((p) => p.text).join("\n").trim();
  }
  function frameOf(el) {
    const frame = isRecord(el.frame) ? el.frame : {};
    return {
      x: bounded(frame.x, 0, -5e4, 5e4),
      y: bounded(frame.y, 0, -5e4, 5e4),
      w: bounded(frame.w, 0, 0, 5e4),
      h: bounded(frame.h, 0, 0, 5e4),
      rotation: bounded(frame.rotation, 0, -360, 360)
    };
  }
  function styleOf(el) {
    return isRecord(el.style) ? el.style : {};
  }
  var SAFE_IMAGE_DATA = /^data:image\/(png|jpeg|gif|webp);base64,[A-Za-z0-9+/=]+$/;
  var HEX_COLOR = /^#(?:[0-9a-fA-F]{3,4}|[0-9a-fA-F]{6}|[0-9a-fA-F]{8})$/;
  var FUNC_COLOR = /^(?:rgb|rgba|hsl|hsla)\(\s*[\d.%\s,/-]+\)$/;
  var NAMED_COLOR = /^[a-zA-Z]{3,20}$/;
  ...
  var ATTACHMENT_PREFIX = "att:";
  function canvasSourceOf(node, resolveCanvas) {
    ...
    if (src.toLowerCase().startsWith(ATTACHMENT_PREFIX) && resolveCanvas) {
      const json2 = resolveCanvas(src.slice(ATTACHMENT_PREFIX.length));
      if (json2 !== void 0) return { json: json2, src };
    }
    return { src };
  }
  function canvasTextLines(node, resolveCanvas) {
    const { json: json2, src } = canvasSourceOf(node, resolveCanvas);
    if (json2 === void 0) return { pages: [], error: src ? `canvas not available: ${src}` : "canvas has no source" };
    const read = readCanvasDocument(json2);
    if (!read.ok) return { pages: [], error: read.error };
    const pageId = typeof node.attrs.page === "string" ? node.attrs.page : void 0;
  ...
  function issuesRequest(node) {
    ...
    return {
      project,
      ...status && ISSUE_STATUSES.includes(status) ? { status } : {},
  ...
  function unavailableMessage(status, what, detail) {
    const base = (() => {
      switch (status) {
        case "missing":
  ...
  function createRenderCtx(doc, options) {
    return {
...
${inner}
  ...
  function withStyleTokens(html, classes) {
    ...
    const merged = classAttr ? rest.replace(classAttr[0], ` class="${classAttr[1]} ${classes.join(" ")}"`) : ` class="${classes.join(" ")}"${rest}`;
    return `${tagStart}${merged}>${html.slice(whole.length)}`;
  }
  function renderDirectiveBlock(node, ctx) {
    const name = node.name;
    const idAttr = node.id ? ` id="${escapeAttr(node.id)}"` : "";
    const variant = variantAttr(node);
    const dataAttrs = Object.entries(node.attrs).filter(([k]) => k !== "id" && k !== "class").map(([k, v]) => ` data-${escapeAttr(k)}="${escapeAttr(String(v))}"`).join("");
    switch (name) {
      case "summary":
      case "abstract":
  ...
  function macroPlaceholder(kind, status, message, idAttr) {
...
${rows}
  ...
  function renderComponentDefinition(node, ctx) {
  ...
  function renderDeck(node, idAttr, ctx) {
  ...
  function renderSlide(node, idAttr, ctx, index, total) {
  ...
  function renderSlideNotes(node, idAttr, ctx) {
    return `<details class="noma-slide-notes"${idAttr}><summary>Speaker notes</summary>${renderChildren(node, ctx)}</details>`;
  }
  var WIDGET_DEFAULT_HEIGHT = 320;
  function renderWidgetFrame(node, idAttr, ctx) {
    if (!ctx.resolveWidgetFrame) return void 0;
    if (!node.id) {
    ...
    const height = Number.isFinite(rawHeight) ? Math.max(40, Math.min(2e3, Math.round(rawHeight))) : WIDGET_DEFAULT_HEIGHT;
  ...
  function renderCanvas(node, idAttr, ctx) {
    const caption = attrValueText(node.attrs, "caption") ?? attrValueText(node.attrs, "title");
    const figcaption = caption ? `<figcaption>${escapeHtml(caption)}</figcaption>` : "";
    const { json: json2, src } = canvasSourceOf(node, ctx.resolveCanvas);
    if (json2 === void 0) {
    ...
    const resolveImage = (value) => {
      ...
      return ctx.externalAssets && /^https:\/\//i.test(value) ? value : void 0;
    };
...
${bodyRows}
  ...
  function renderDiagram(node, idAttr) {
    ...
    const placeholder2 = `<pre class="noma-diagram-source">${escapeHtml(body)}</pre>`;
    const figure = `<div class="${cls}" data-noma-source="${escapeAttr(body)}">${placeholder2}</div>`;
  ...
  function renderChartSvg(seriesList, type2, w, h, labels, labelOptions) {
    ...
    if (type2 === "bar") {
      ...
      plot = seriesList.map((ser, sIdx) => {
        const color2 = PLOT_COLORS[sIdx % PLOT_COLORS.length];
        const points = ser.values.map((v, i) => `${x(i).toFixed(1)},${y(v).toFixed(1)}`).join(" ");
        const areaFill = sIdx === 0 && nSeries === 1 ? `<path d="M ${x(0).toFixed(1)},${(padT + innerH).toFixed(1)} L ${points.split(" ").join(" L ")} L ${x(N - 1).toFixed(1)},${(padT + innerH).toFixed(1)} Z" fill="${color2}" opacity="0.12" />` : "";
        const line = `<polyline points="${points}" fill="none" stroke="${color2}" stroke-width="2" />`;
        const markers = showMarkers ? ser.values.map(
          (v, i) => `<circle cx="${x(i).toFixed(1)}" cy="${y(v).toFixed(1)}" r="2.5" fill="${color2}" />`
        ).join("") : "";
    ...
    if (showLegend) {
      ...
      legend = seriesList.map((ser, sIdx) => {
        ...
        cursor += labelW;
        return `<rect x="${swatchX}" y="6" width="10" height="10" fill="${color2}" opacity="0.85" /><text x="${textX}" y="14" font-size="${FONT_PX}" fill="currentColor" opacity="0.85">${escapeHtml(ser.name)}</text>`;
      }).join("");
    }
...
`;
  ...
  function emit(node, out, depth, opts, forceSubtree) {
    switch (node.type) {
      ...
      case "table": {
        ...
        const fmt2 = (cells) => "| " + cells.map((c, i) => c.padEnd(widths[i] ?? c.length)).join(" | ") + " |";
        out.push(fmt2(node.header));
        out.push(
          "| " + widths.map((w, i) => {
        ...
        for (const row of node.rows) out.push(fmt2(row));
        out.push("");
        return;
      }
      case "directive":
  ...
  function emitDirective(node, out, depth, opts, forceSubtree) {
    ...
    } else if (VERBATIM_BODY.has(node.name) && node.body !== void 0) {
      out.push(node.body);
  ...
  var LAYOUT_CONTAINERS = /* @__PURE__ */ new Set(["grid", "columns", "tabs", "accordion", "hero", "deck"]);
...
$$`, ctx);
  ...
  function renderSlideMarkdown(node, ctx, depth) {
    if (node.name === "notes") {
  ...
  function metadataParts(node) {
    const skip = /* @__PURE__ */ new Set(["id", "title", "caption", "label", "Label", "name", "src", "alt", "class"]);
    const parts = [];
...
${fence}`;
  ...
  var DECK_DIRECTIVES = ["deck", "slide", "notes"];
  var COMPONENT_DIRECTIVES = /* @__PURE__ */ new Set(["component", "slot"]);
  for (const name of ["technical", "research"]) PROFILES[name] = /* @__PURE__ */ new Set([...PROFILES[name], ...DECK_DIRECTIVES]);
  ...
  PROFILES.presentation = /* @__PURE__ */ new Set([
    ...minimalProfile,
    ...DECK_DIRECTIVES,
    "hero",
    "grid",
  ...
  function validateCanvas(node, diagnostics) {
    const at = { pos: node.pos, ...node.endLine ? { endLine: node.endLine } : {}, ...node.id ? { nodeId: node.id } : {} };
    const { json: json2, src } = canvasSourceOf(node);
    if (json2 === void 0) {
  ...
  function validate(source, options = {}) {
    ...
    for (const node of walk(doc)) {
      ...
      if (node.name === "claim" && !suppressed(node) && "confidence" in node.attrs) {
        ...
        let num2 = null;
        if (typeof c === "number" && Number.isFinite(c)) num2 = c;
        else if (typeof c === "string" && c.trim() !== "") {
          const n = Number(c);
          if (Number.isFinite(n)) num2 = n;
        }
        if (num2 === null || num2 < 0 || num2 > 1) {
          diagnostics.push({
            severity: "warning",
  ...
  function withoutComponentTemplates(doc) {
    const strip = (nodes) => {
      let changed = false;
      const out = nodes.map((node) => {
        if (node.type === "directive" && node.name === "component") {
          changed = true;
          return { ...node, children: [], body: void 0 };
        }
  ...
  function validateComponents(doc, host) {
    const diagnostics = [];
    const seen = /* @__PURE__ */ new Set();
  ...
  function validateDecksAndStyleTokens(nodes, parent, diagnostics, aliases) {
    for (const node of nodes) {
      if (node.type === "directive" && !suppressed(node)) {
        ...
        if (node.name === "slide") {
          ...
          if (node.attrs.layout !== void 0 && !isSlideLayout(node.attrs.layout)) {
            diagnostics.push({
              severity: "warning",
        ...
        if (node.name === "deck" && node.attrs.aspect !== void 0 && !(typeof node.attrs.aspect === "string" && DECK_ASPECTS[node.attrs.aspect])) {
          diagnostics.push({
            severity: "warning",
  ...
  function validateMacroNode(node, diagnostics, options, referenceBlock) {
    ...
    switch (node.name) {
      ...
      case "issues": {
        ...
        if (status !== void 0 && !ISSUE_STATUSES.includes(status)) {
...
`;
  ...
  function wrapSelection(prefix, suffix, placeholder2) {
    const selection = sourceSelection();
    const body = selection.text || placeholder2;
  ...
  function wrapBlock(prefix, suffix, placeholder2) {
    const selection = sourceSelection();
    const body = selection.text || placeholder2;
    const inserted = `${prefix}${body}${suffix}`;
    replaceRange(selection.start, selection.end, inserted, selection.start + prefix.length, selection.start + prefix.length + body.length);
...
body { background: #ffffff; }`;
  ...
  function jsonDatasetForEditor(node, parsed) {
    if (Array.isArray(parsed)) {
      if (parsed.length > 0 && isRecord2(parsed[0])) {
        const columns2 = Object.keys(parsed[0]);
        return {
          columns: columns2,
          rows: parsed.filter(isRecord2).map((row) => columns2.map((column) => scalarText(row[column])))
        };
      }
      const rows2 = parsed.filter(Array.isArray).map((row) => row.map(scalarText));
      return { columns: columnsAttr2(node, rows2), rows: rows2 };
    }
    const record = isRecord2(parsed) ? parsed : null;
  ...
  function yamlDatasetForEditor(node, parsed) {
    ...
    if (!record || !Array.isArray(record.rows)) return null;
    const rows = record.rows.filter(Array.isArray).map((row) => row.map(scalarText));
    const schema2 = isRecord2(record.schema) ? record.schema : null;
