...
(() => {
  ...
  function readDOMChange(view, from3, to, typeOver, addedNodes) {
    ...
    let sel = view.state.selection;
    let parse = parseBetween(view, from3, to, addedNodes);
...
${err.toString()}`);
  ...
  var updateYText = (ytext, ptexts, meta2) => {
    ...
    const content = ptexts.map((p) => ({
      ...
      attributes: Object.assign({}, nAttrs, marksToAttributes(p.marks, meta2))
    }));
  ...
  RopeSequence.prototype.forEach = function forEach3(f, from3, to) {
    if (from3 === void 0) from3 = 0;
    if (to === void 0) to = this.length;
    if (from3 <= to) {
  ...
  var Branch = class _Branch {
    ...
    addMaps(array) {
      if (this.eventCount == 0)
        return this;
      return new _Branch(this.items.append(array.map((map3) => new Item2(map3))), this.eventCount);
    }
  ...
  function mountHostedCollab(options) {
    ...
    const connect = () => {
      ...
      socket.addEventListener("open", () => setStatus2("connected"));
      socket.addEventListener("message", (event) => {
        const message = JSON.parse(String(event.data));
        if ((message.type === "init" || message.type === "update") && message.update) {
          applyUpdate(ydoc, b64ToBytes(message.update), "remote");
          ready = true;
          ensureEditor();
  ...
  // web/enterprise-workspace.ts
  var STORAGE_KEY = "noma.enterprise.session.v1";
  var params2 = new URLSearchParams(window.location.search);
  var token = params2.get("token") ?? localStorage.getItem(STORAGE_KEY) ?? "";
  var mode = params2.get("mode") || "docs";
  var payload;
  ...
  var gate = $2("login-gate");
  var statusEl = $2("session-status");
  var railList = $2("rail-list");
  var inspector = $2("inspector");
  var searchResults = $2("search-results");
  var editorMount = $2("editor");
  function meta(name) {
    return document.querySelector(`meta[name="${name}"]`)?.getAttribute("content") ?? "";
  }
  function setStatus(text2, state = "connecting") {
  ...
  function setMode(next) {
    ...
    for (const canvas of document.querySelectorAll(".ew-canvas")) {
  ...
  async function signIn(tenantId, idToken) {
  ...
  $2("visual-outline").addEventListener("click", (event) => {
    ...
    const id2 = button?.dataset.frame;
    if (!id2) return;
    for (const node of document.querySelectorAll(".pd-el")) node.classList.toggle("is-selected", node.getAttribute("data-id") === id2);
  });
  $2("workspace-search").addEventListener("input", async (event) => {
    ...
    searchResults.innerHTML = result.hits.map(
  ...
  $2("login-submit").addEventListener("click", () => {
    void signIn($2("login-tenant").value.trim(), $2("login-token").value.trim()).catch((error) => {
      $2("login-error").textContent = error instanceof Error ? error.message : String(error);
    });
  });
  $2("login-demo").addEventListener("click", () => {
    void signIn(meta("noma-tenant-id") || $2("login-tenant").value.trim(), meta("noma-demo-user") || "alice").catch(
      (error) => {
        $2("login-error").textContent = error instanceof Error ? error.message : String(error);
      }
    );
  ...
  if (token) {
    ...
    $2("login-tenant").value = tenant;
    $2("login-token").value = demoUser;
    if (tenant && demoUser && params2.get("autologin") !== "0") {
