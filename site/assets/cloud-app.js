...
::`;
  ...
  async function createWorkIssue() {
    ...
    try {
      const issue = await fetchCloudJson(`/api/projects/${encodeURIComponent(project.id)}/issues`, {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({
          summary: summary2,
          ...
          labels: issueLabelsInput.value.trim() || void 0,
          sprintId: issueSprintSelect.value || void 0
        })
      });
  ...
  async function createWorkSprint() {
    const project = selectedWorkProject();
    ...
    try {
      const sprint = await fetchCloudJson(`/api/projects/${encodeURIComponent(project.id)}/sprints`, {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  async function selectWorkIssue(issueId) {
    const project = selectedWorkProject();
    if (!project) return;
    try {
  ...
  function renderWorkSprintSelects() {
    ...
    for (const sprint of state.workSprints) {
      ...
      if (sprint.status !== "closed") {
        ...
        issueOption.textContent = `${sprint.name} \xB7 ${sprint.status}`;
  ...
  function workIssueRow(issue) {
    ...
    title.textContent = `${issue.key} \xB7 ${issue.summary}`;
    const meta = document.createElement("span");
    meta.className = "row-meta";
    meta.textContent = `${issue.type} \xB7 ${issue.priority}${issue.assigneeName ? ` \xB7 ${issue.assigneeName}` : ""}`;
    ...
    try {
      const response = await fetchCloudJson("/api/auth/register", {
        ...
        body: JSON.stringify({
          ...
          invitationCode: cloudInvitationCodeInput.value || void 0
        })
      });
  ...
  async function loginCloudUser() {
    ...
    try {
      const response = await fetchCloudJson("/api/auth/session", {
        method: "POST",
        headers: { "content-type": "application/json" },
  ...
  async function askNoma() {
    ...
    try {
      state.askNomaResponse = await fetchCloudJson("/api/ask", {
        method: "POST",
        headers: { "content-type": "application/json" },
        body: JSON.stringify({ query: query2, ...state.currentSite ? { siteId: state.currentSite.id } : {}, ...aiAskMode() ? { mode: aiAskMode() } : {} })
      });
      setPanelStatus(
        askNomaStatus,
...
Start writing here.`;
  ...
  // web/cloud/ai.ts
  var aiMenu = requireElement3("aiMenu");
  var aiMenuButton = requireElement3("aiMenuButton");
  var aiMenuItems = requireElement3("aiMenuItems");
  var aiMenuStatus = requireElement3("aiMenuStatus");
  var aiSummarizeButton = requireElement3("aiSummarizeButton");
  var aiDraftButton = requireElement3("aiDraftButton");
  var aiRefreshButton = requireElement3("aiRefreshButton");
  var aiGenerateToggle = requireElement3("aiGenerateToggle");
  var aiAssistantSection = requireElement3("aiAssistantSection");
  var aiAssistantStatus = requireElement3("aiAssistantStatus");
  var aiAssistantOutput = requireElement3("aiAssistantOutput");
  var maintenanceSection = requireElement3("aiMaintenanceSection");
  var maintenanceDetails = requireElement3("aiMaintenanceDetails");
  var maintenanceEnabled = requireElement3("aiMaintenanceEnabled");
  var maintenanceRefresh = requireElement3("aiMaintenanceRefresh");
  var maintenanceInterval = requireElement3("aiMaintenanceInterval");
  var maintenanceLimit = requireElement3("aiMaintenanceLimit");
  var maintenanceSaveButton = requireElement3("aiMaintenanceSaveButton");
  var maintenanceRunButton = requireElement3("aiMaintenanceRunButton");
  var maintenanceStatus = requireElement3("aiMaintenanceStatus");
  var maintenanceItems = requireElement3("aiMaintenanceItems");
  var aiState = { items: [], running: false };
  function aiAskMode() {
    return aiGenerateToggle.checked ? "generative" : void 0;
  }
  function renderAnswerText(container, text, citationNumbers, openCitation) {
    container.textContent = "";
    ...
    for (const match of text.matchAll(/\[(\d{1,3})\]/g)) {
      ...
      if (!known.has(number) || match.index === void 0) continue;
  ...
  function answerNote(response) {
    ...
    if (response.mode !== "generative" || !response.generation) return void 0;
    ...
    return `Generated by ${response.generation.model ?? "the model"} from the cited blocks only${dropped ? ` \xB7 ${dropped} unverifiable citation${dropped === 1 ? "" : "s"} removed` : ""}.`;
  }
  function installCloudAi() {
  ...
  function renderAiChrome() {
    ...
    if (aiState.statusUserId && aiState.statusUserId !== state.cloudUser?.id) {
      aiState.status = void 0;
      aiState.statusUserId = void 0;
    }
  ...
  function syncMaintenance() {
    const key = state.cloudAvailable && state.cloudUser && state.currentSite ? `${state.cloudUser.id}:${state.currentSite.id}` : void 0;
  ...
  async function summarizePage() {
    await runAiAction("Summarizing this page", async () => {
      ...
      setPanelStatus(aiAssistantStatus, `Summary by ${response.model} \xB7 $${response.costUsd.toFixed(4)}`, "ok");
    });
  }
  async function insertSummary() {
    await runAiAction("Drafting a summary block proposal", async () => {
      const response = await postJson(pageAiEndpoint("summarize"), { insert: true });
  ...
  async function draftChanges() {
    ...
    await runAiAction("Drafting changes as a proofed proposal", async () => {
      const response = await postJson(pageAiEndpoint("draft"), { instruction });
  ...
  async function refreshFromSources() {
    ...
    await runAiAction("Reading sources and drafting a refresh proposal", async () => {
      const response = await postJson(pageAiEndpoint("refresh"), { sourceUrls });
  ...
  async function openProposal(proposal, message) {
    await refreshPatchProposals();
    const summary2 = document.createElement("div");
    summary2.className = "knowledge-answer-text ai-answer-text";
    summary2.textContent = `${proposal.summary ?? "AI proposal"}

Another collaborator must approve it in Agent Review before it can be applied.`;
    aiAssistantOutput.append(summary2);
    setPanelStatus(aiAssistantStatus, `${message} \xB7 awaiting independent review`, "ok");
    if (!state.panelsOpen) {
  ...
  async function loadMaintenance(siteId, key = aiState.maintenanceKey) {
    aiState.maintenanceLoading = true;
    try {
      const [settings, items] = await Promise.all([
        fetchCloudJson(`/api/sites/${encodeURIComponent(siteId)}/maintenance`),
        fetchCloudJson(`/api/sites/${encodeURIComponent(siteId)}/maintenance/items?status=open`)
      ]);
  ...
  function renderMaintenance() {
    ...
    const aiNote = data.ai.available ? "" : ` \xB7 AI drafts off (${(data.ai.reason ?? "").replaceAll("_", " ")})`;
    setPanelStatus(
      maintenanceStatus,
      `${data.openItems} open item${data.openItems === 1 ? "" : "s"}${lastRun ? ` \xB7 last ${lastRun.trigger} sweep ${formatDate(lastRun.startedAt)}: ${lastRun.proposalsCreated} draft${lastRun.proposalsCreated === 1 ? "" : "s"}` : " \xB7 never swept"}${aiNote}`,
    ...
    for (const item of aiState.items.slice(0, 12)) {
      ...
      const row = collaborationRow(item.kind.replaceAll("_", " "), item.message, `${page?.title ?? "page"}${item.blockId ? ` \xB7 #${item.blockId}` : ""}${item.proposalId ? " \xB7 refresh drafted" : ""}`);
  ...
  async function saveMaintenance() {
    ...
    if (!siteId) return;
    try {
      aiState.maintenance = await putJson(`/api/sites/${encodeURIComponent(siteId)}/maintenance`, {
  ...
  async function runMaintenance() {
    ...
    try {
      const result = await postJson(`/api/sites/${encodeURIComponent(siteId)}/maintenance/run`, {});
  ...
  function postJson(url, body) {
    return fetchCloudJson(url, { method: "POST", headers: { "content-type": "application/json" }, body: JSON.stringify(body) });
  }
