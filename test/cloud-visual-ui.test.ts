...
test("two browsers co-edit one page in Visual mode and see each other's edits and presence", { timeout: 90_000 }, async (t) => {
  ...
  const aliceSession = await alice.evaluate(() => JSON.parse(localStorage.getItem("noma.cloud.user.v1") ?? "{}") as BrowserSession);
  ...
  await bobPage.evaluateOnNewDocument((session) => localStorage.setItem("noma.cloud.user.v1", JSON.stringify(session)), { id: bob.id, name: bob.name, token: bob.token });
...
test("visual mode inserts blocks from the slash menu and markdown shortcuts without a live connection", { timeout: 60_000 }, async (t) => {
  ...
  const session = await page.evaluate(() => JSON.parse(localStorage.getItem("noma.cloud.user.v1") ?? "{}") as BrowserSession);
