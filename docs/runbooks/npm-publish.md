...
Manual steps to publish a Noma release to npm. Run after the release tag exists on `origin/main` (e.g. `v0.11.1`) and the GitHub release is live.
...
# Close the v0.11.1 GH milestone if one exists
gh issue list --label release --search "v0.11.1" --state open
```
