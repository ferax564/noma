...
- Noma Cloud server (`npm run build:cloud && npm start`) — serves the cloud app, workbench, and rendered sites while persisting users, documents, immutable revisions, spaces, direct/group permissions, comments, mentions, notifications, approvals, activity, projects, issues, boards, sprints, proofed patch proposals, share links, diagnostics, FTS5 block search, per-user recents/favorites, recoverable trash, JSON/LLM exports, and reader artifacts in SQLite. Conflict-safe writes require the hash a client loaded. Documents live behind `/api/documents`; workspaces live behind `/api/sites`; integrated work lives behind `/api/projects`; reader artifacts render at `/d/<id>` and `/s/<id>`.
- Noma Cloud app (`site/cloud.html`, published as `dist/cloud.html`) — hosted workspace editor combining Confluence-style spaces, page navigation, search, templates/import, favorites/recents/trash, comments, notifications, activity, groups, approvals, and history with Jira-style projects, issue boards, backlog/sprints, assignees, labels, links, and issue comments. Agent patch proposals are server-proofed, can link to issues, require independent editor approval, and reject stale versions before apply.
