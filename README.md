...
**v0.5.1** — closes the two items deferred from review fixes #5 and #6. New VS Code language extension at [`tools/vscode-noma/`](tools/vscode-noma/) — TextMate grammar (`source.noma`) for directive blocks, attributes, wikilinks, math, embedded YAML/JSON/Mermaid/DOT/LaTeX, escape-hatch warnings, folding markers. Install locally with `vsce package` → `code --install-extension` (not yet on the marketplace). Killer demo at [`examples/agent-stale-memo/`](examples/agent-stale-memo/) — a research memo with two stale citations and a low-confidence claim, refreshed by five block-level patch operations; runner validates before, applies via `patchSource`, validates after (clean), and writes a narrated `trace.html`. ~89% of source lines stay byte-identical. Run with `npm run demo:stale-memo`. **v0.5** — closes the executive-read review (#1–#4) plus issues #10/#11/#12. Source-preserving `patchSource` (frontmatter, sibling blocks, blank-line padding all survive byte-for-byte); `renderNoma` emits explicit heading id/aliases so roundtrip is stable; wikilink grammar accepts `/`, `.`, `:` in IDs; `@noma/cli` now exposes a programmatic API (`import { parse, patchSource, renderHtml }`); new `::diagram{kind="mermaid|graphviz|drawio"}`, `::plotly`, and `::dataset{src="..."}` directives with per-kind CDN auto-injection. **v0.4.1** — site-index polish (issue #9): card descriptions parse `**bold**`, `` `code` ``, `*em*`, links, and wikilinks instead of emitting them literally; the index page now carries the same `nav.noma-site-nav` element as chapter pages; description truncation honours sentence boundaries. **v0.4** — multi-page book site renderer (`--to site`), KaTeX math (`::math` and `$..$` delimiters), scoped heading IDs in book mode, chapter aliases (filename slug + frontmatter `aliases:`), composable profiles (`profiles: [research, technical]`), `--ignore-rule` flag, `prepare` build hook so `npm i -g github:ferax564/noma` works — all shipped in response to issues #2 through #8. **v0.3** shipped `::table`, `noma fmt`, `::state_change`, plot/dataset linkage, profile frontmatter, wikilink validation across chapters, citation staleness overrides. **v0.2** shipped `noma patch`, the source printer, theme variants, real plots, book manifests, and escape hatches. See [`CHANGELOG.md`](CHANGELOG.md) and `PLAN.md` §24 for the per-version tracker. Out of scope for now: visual editor, realtime collaboration, plugin marketplace, hosted SaaS. The point is to ship a useful core and let the community shape what comes next.
