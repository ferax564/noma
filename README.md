...
**Status:** v0.9.0 — debuts `@noma/agent-sdk` v0.1.0, the experimental reference Agent SDK that closes the v1.1 RFC Annex A+B graduation prerequisite. Two public classes: `NomaTools` (1:1 wrapper over the four `@noma/mcp-server` MCP tools, exercised via stdio) and `NomaWorkflow` (composes them into safe-patch with per-file mutex + clamped retry, client-side op chains with parent transcripts, transcript replay, and advisory capability checks including the Annex A `ids.rename` global gate). Error channel split: thrown `NomaSystemError` hierarchy for system faults (including book-manifest `unsupported_op`); returned `{ ok: false, code }` body for user-recoverable §3.5 patch errors. Five-tier test pyramid (unit / tools-vs-real-server / workflow / demo replay / conformance) feeds three graduation metrics — all three at target in this release: 7/7 single-call error codes, 8/8 Annex A.3 descriptor fields, 6/6 conformance fixtures pass through the SDK end-to-end with byte-identical output to the non-SDK baselines. SDK API is marked **experimental**; freezes at v1.0 in lockstep with RFC v1.1 graduation. Carries everything from v0.8.x. See [`packages/agent-sdk/README.md`](packages/agent-sdk/README.md), [`CHANGELOG.md`](CHANGELOG.md), and `PLAN.md` §24.14 for the full per-version tracker.
