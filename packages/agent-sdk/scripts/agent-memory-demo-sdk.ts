...
const FIXTURE = resolve(REPO_ROOT, "examples/agent-memory");
const OUT_DIR = resolve(REPO_ROOT, "dist/examples/agent-memory-sdk");
...
export async function runDemo(): Promise<{
  ...
  const src = resolve(FIXTURE, "memory.noma");
  const dst = resolve(OUT_DIR, "memory.after.noma");
...
function renderTrace(results: Awaited<ReturnType<NomaWorkflow["applyOps"]>>): string {
  const rows = results
    ...
    .join("\n");
  return `<!doctype html><meta charset="utf-8"><title>agent-memory-sdk</title><table border="1"><thead><tr><th>#</th><th>result</th><th>op</th><th>validation</th></tr></thead><tbody>${rows}</tbody></table>`;
}
